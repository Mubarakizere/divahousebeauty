...
class CategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($minPrice !== null && $maxPrice !== null && $minPrice <= $maxPrice) {
            $productsQuery->whereBetween('standard_price', [$minPrice, $maxPrice]);
        } elseif ($minPrice !== null) {
            $productsQuery->where('standard_price', '>=', $minPrice);
        } elseif ($maxPrice !== null) {
            $productsQuery->where('standard_price', '<=', $maxPrice);
        }

        // sorting
        switch ($sortby) {
            case 'price_asc':  
                $productsQuery->orderBy('standard_price', 'asc'); 
                break;
            case 'price_desc': 
                $productsQuery->orderBy('standard_price', 'desc'); 
                break;
            case 'name':       
